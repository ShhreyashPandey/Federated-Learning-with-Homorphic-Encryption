...
int main() {
    try {
        ...
        if (!skFile) {
            std::cerr << "[c1_decrypt] ERROR: could not open private key\n";
            return 1;
        }
        ...
        if (!resp_json.contains("agg_params")) {
            std::cerr << "[c1_decrypt] ERROR: agg_params missing in server response\n";
            return 1;
        }
        ...
        std::cout << "[c1_decrypt] Decrypted aggregated params: w=" << w << ", c=" << c << std::endl;
        ...
        std::cout << "[c1_decrypt] Saved decrypted aggregated params to agg_wc.json\n";
    }
