...
int main() {
    try {
        ...
        cout << "[Client2 Rekey] Using round " << round << endl;

        // STEP 1: Load CryptoContext
        cout << "[Client2 Rekey] Loading CryptoContext..." << endl;
        ...
        cout << "[Client2 Rekey] CryptoContext loaded" << endl;

        // STEP 2: Load private key
        cout << "[Client2 Rekey] Loading private key..." << endl;
        PrivateKey<DCRTPoly> sk;
        ifstream skIn("client2_private.key", ios::binary);
        if (!skIn.is_open()) {
            throw runtime_error("Failed to open client2_private.key");
        }
        Serial::Deserialize(sk, skIn, SerType::BINARY);
        cout << "[Client2 Rekey] Private key loaded" << endl;

        // STEP 3: Fetch ciphertexts
        cout << "[Client2 Rekey] Fetching ciphertext metadata from server..." << endl;
        string url = "http://localhost:8000/encrypted-data?round=" + to_string(round);
        string responseStr = HttpGetJson(url);
        cout << "[Client2 Rekey] Raw JSON size: " << responseStr.size() << " bytes" << endl;
        ...
        cout << "[Client2 Rekey] JSON parsed" << endl;
        ...
        for (const auto& ct : response["ciphertexts"]) {
            ...
            if (clientId == "client1") {
                ...
                cout << "[Client2 Rekey] Found client1 public key. Base64 size: " << pk_b64.size() << " bytes" << endl;

                if (pk_b64.empty()) {
                    throw runtime_error("Client1 public key is empty.");
                }

                try {
                    pk2 = DeserializePublicKeyFromBase64(pk_b64);
                } catch (const std::exception& ex) {
                    cerr << "Failed to deserialize client1 public key: " << ex.what() << endl;
                    throw;
                }
        ...
        if (!found) throw runtime_error("Public key for client1 not found");

        // STEP 5: Generate and POST rekey
        cout << "[Client2 Rekey] Generating ReKey..." << endl;
        ...
        cout << "[Client2 Rekey] Posted rekey to server: " << postResp << endl;
        cout << "-----------------------------------------------------\n";

    } catch (const std::exception& e) {
        cerr << "[Client2 Rekey] Error: " << e.what() << endl;
        cout << "-----------------------------------------------------\n";
        return 1;
    }
