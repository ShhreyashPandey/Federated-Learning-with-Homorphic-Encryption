...
def test_and_post_accuracy(test_csv, model_pickle_path, accuracy_json_path, accuracy_log_csv, server_url, client_id="client1"):
    # Load test data
    ...
    try:
        ...
        print(f"[c1_test.py] Error loading model pickle: {e}")
        return
    ...
    print(f"[c1_test.py] Test MSE: {mse}. Saved to {accuracy_json_path}")

    # Append to CSV for all rounds
    ...
    try:
        ...
        print(f"[c1_test.py] POST accuracy response: Status {response.status_code}, Body: {response.text}")
    except requests.RequestException as e:
        print(f"[c1_test.py] POST request failed: {e}")

if __name__ == "__main__":
    # python client1_test.py <test_csv> <model_pickle> <accuracy_json> <server_url> [<accuracy_log_csv>]
    if len(sys.argv) < 5:
        print("Usage: python client1_test.py <test_csv> <model_pickle> <accuracy_json> <server_url> [<accuracy_log_csv>]")
        sys.exit(1)
    ...
    # Default CSV log path is client1_data/accuracy_log.csv
    if len(sys.argv) >= 6:
